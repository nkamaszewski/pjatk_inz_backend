...
exports.updateParticipation = async (req, res, next) => {
	if (req.userIdRole != Role.ADMIN && req.userId != req.body.IdPerson) {
		res.status(403).json({
			message: 'Brak uprawnień',
		});
	} else {
		...
		if (req.userId == req.body.IdPerson) {
			ParticipationRepository.updateParticipationUser(participId, data)
				.then((result) => {
					res
						.status(200)
						.json({ message: 'Participation updated!', particip: result });
				...
				.catch((err) => {
					...
					res.status(403).json({
						message: `Nie udało się przesłać certyfikatu`,
					});
				});
		}
	}
